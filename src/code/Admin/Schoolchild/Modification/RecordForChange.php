...
<h1></h1>
<p><strong><h1><center>Изменение успеваемости школьника</center></strong></h1></p>
<center>
<img src="../../../../materials/images/change.png" width="80" height="80" >
</center>
<h1></h1>
<?php 
...
echo "<tr align=center valign=middle bgcolor=#2d5787 style='color:#ffffff'><td>Фамилия</td><td>Имя</td><td>Предмет</td><td>Название оценки</td><td>Оценка</td><td>Изменение</td></tr>"; 
$result = mysqli_query($conn_id, "SELECT schoolchild.id_schoolchild, schoolchild.surname, schoolchild.first_name, subject.name_subject, mark.name_mark, academicrecord.mark, academicrecord.id_academicrecord  FROM schoolchild LEFT JOIN school.academicrecord ON schoolchild.id_schoolchild = academicrecord.id_schoolchild LEFT JOIN school.mark ON academicrecord.id_mark = mark.id_mark LEFT JOIN school.subject ON academicrecord.id_subject = subject.id_subject WHERE schoolchild.id_schoolchild =$id") or exit ();
while ($row = mysqli_fetch_assoc($result)) {
	...
	echo "<td>".$row['mark']."</td>";
	echo "<td><a href='ModificationRecord.php?id_academicrecord=".$row['id_academicrecord']."&id_schoolchild=".$row['id_schoolchild']."'>Изменение</a></td>";
	echo "</tr>";
}
echo "</table>"; 
